...
vk::Extent2D setExtent(const vk::SurfaceCapabilitiesKHR& capabilities, const std::pair<uint32_t, uint32_t>& glfw_framebuffer_size);


namespace vulkan {

	Swapchain::Swapchain(const Device& device, const vk::raii::SurfaceKHR& surface, const std::pair<uint32_t, uint32_t>& glfw_framebuffer_size, uint32_t image_count) :
		m_handle{ nullptr }
	{

		SwapchainSupport device_support{ device.swapchainSupport() };
		...
		QueueIndices indices{ device.indices() };
		...
		try {
			m_handle = (*device).createSwapchainKHR(swapchain_info);
		}
		...
		for (const auto& image : swapchain_images) {
			...
			try {
				m_image_views.emplace_back((*device).createImageView(image_view_info));
			}
			catch (const vk::SystemError& err) {
...
vk::Extent2D setExtent(const vk::SurfaceCapabilitiesKHR& capabilities, const std::pair<uint32_t, uint32_t>& glfw_framebuffer_size) {
	...
	return vk::Extent2D{
			.width  = std::clamp(glfw_framebuffer_size.first, capabilities.minImageExtent.width, capabilities.maxImageExtent.width),
			.height = std::clamp(glfw_framebuffer_size.second, capabilities.minImageExtent.height, capabilities.maxImageExtent.height),
	};
